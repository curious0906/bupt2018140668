    ...
    <table class="table table-hover">
        ...
        <c:forEach items="${allRoles}" var="role" varStatus="vs">
            <tr>
                ...
                <td>${role.characterCode}</td>
                <td>${role.characterDescription}</td>
                <td><fmt:formatDate value="${role.characterCreated}" pattern="yyyy-MM-dd"/></td>
                <td><fmt:formatDate value="${role.characterUpdated}" pattern="yyyy-MM-dd"/></td>
                <td>
                    <button type="button" class="btn btn-xs btn-default" onclick="toEditPrivilege(${role.characterId})">
                        <span class='glyphicon glyphicon-wrench'></span> 权限
                    </button>
                    <button type="button" class="btn btn-xs btn-info" onclick="toEditRole(${role.characterId})">
                        <span class="glyphicon glyphicon-pencil"></span> 编辑
                    </button>
                    <button type="button" class="btn btn-xs btn-danger" onclick="deleteRole(${role.characterId})">
