...
<div class="container" style="border: 1px solid #CCCCCC;height: 1000px;">
    ...
    <table class="table table-hover">
        ...
        <c:forEach items="${stores}" var="store" varStatus="vs">
            <tr>
                <td>${vs.count}</td>
                <td>${store.shopManagerId}</td>
                <td>${store.shopName}</td>
                <td>${store.shopPhoneNumber}</td>
                <td>${store.shopTelephone}</td>
                <td>${store.shopPosition}</td>
                <td><fmt:formatDate value="${store.shopCreated}" pattern="yyyy-MM-dd" /></td>
                <td><fmt:formatDate value="${store.shopUpdated}" pattern="yyyy-MM-dd" /></td>
                <td>
                    <button type="button" class="btn btn-xs btn-info" onclick="editStore(${store.shopId})">
                        <span class="glyphicon glyphicon-pencil"></span> 编辑
                    </button>
                    <button type="button" class="btn btn-xs btn-danger" onclick="deleteStore(${store.shopId})">
