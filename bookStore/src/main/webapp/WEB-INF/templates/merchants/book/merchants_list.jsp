...
<head>
    ...
    <script type="text/javascript">
        ...
        function editBook(bookId) {
            location.href = "<%=basePath%>merchants/book/echo?bookId=" + bookId;
        }

        function changeBookShelf(bookId, isShelf) {
            if (isShelf == 1) {
                if (confirm("下架书籍?")) {
                    location.href = "<%=basePath%>merchants/book/shelf?bookId=" + bookId + "&isShelf=0&"+"keywords="+'${keywords}';
                }
            }
            if (isShelf == 0) {
                if (confirm("上架书籍?")) {
                    location.href = "<%=basePath%>merchants/book/shelf?bookId=" + bookId + "&isShelf=1&"+"keywords="+'${keywords}';
                }
            }
        }
        ...
        function deleteBook(bookId) {
            if (confirm("删除这本书?")) {
                location.href = "<%=basePath%>merchants/book/deletion/"+bookId+"?keywords="+'${keywords}';
            }
        }
...
<div class="container" style="border: 1px solid #CCCCCC;">
    <div id="searchBook" style="height:100px;border-bottom: 1px solid #CCCCCC;">
        <form action="merchants/book/list" class="form-inline" role="form" method="get">
    ...
    <table class="table table-hover">
        ...
        <c:forEach items="${bookPageInfo.list}" var="book" varStatus="vs">
            <tr>
                ...
                <td>${book.bookName}</td>
                <td>
                    <img src="${book.bookImageUrl}" style="width: 50%">
                </td>
                <td>${book.bookAuthor}</td>
                <td>${book.bookIsbn}</td>
                <td class="red">￥${book.bookPrice}</td>
                <td class="red">￥${book.bookMarketPrice}</td>
                <td class="red">${book.bookIsShelf == 1 ?"上架中":"已下架"}</td>
                    <%--<td><fmt:formatDate value="${book.publishDate}" pattern="yyyy-MM-dd"/></td>--%>
                <td>${book.bookShopMount}</td>
                <td>
                    <button type="button" class="btn btn-xs btn-info" onclick="editBook(${book.bookId})">
                        <span class="glyphicon glyphicon-pencil"></span> 编辑
                    </button>
                    <button type="button" class="btn btn-xs btn-warning"
                            onclick="changeBookShelf(${book.bookId},${book.bookIsShelf})">
                            ${book.bookIsShelf == 1 ?"<span class='glyphicon glyphicon-arrow-down'></span> 下架":"<span class='glyphicon glyphicon-arrow-up'></span> 上架"}
                    </button>
    ...
    <ul class="pagination pagination-lg">
        ...
        <c:if test="${!bookPageInfo.isFirstPage}">
            <li>
                <a href="merchants/book/list?keywords=${keywords}&page=${bookPageInfo.prePage}">&laquo;</a>
            </li>
        </c:if>
        <c:forEach
            ...
            <li
                ...
                <a href="merchants/book/list?keywords=${keywords}&page=${current}">
                        ${current}
                </a>
            </li>
        </c:forEach>
        ...
        <c:if test="${!bookPageInfo.isLastPage}">
            <li><a href="merchants/book/list?keywords=${keywords}&page=${bookPageInfo.nextPage}">&raquo;</a>
            </li>
        </c:if>

        <li>
            <a href="merchants/book/list?keywords=${keywords}&page=${bookPageInfo.pages}">末页</a>
        </li>
