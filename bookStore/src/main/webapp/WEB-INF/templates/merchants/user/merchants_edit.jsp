...
<div class="container" style="border: 1px solid #CCCCCC;height: 1000px;">
    ...
    <form class="form-horizontal" role="form" id="storeForm" method="post" action="merchants/user/update">
        <input type="hidden" name="userId" value="${user.userId}">
        <input type="hidden" name="username" value="${user.userUsername}">
        <div class="form-group">
            <label for="username" class="col-sm-2 control-label">用户名：</label>
            <div class="col-sm-5">
                <span id="username">${user.userUsername}</span>
            </div>
            <span></span>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="text" class="form-control" id="nickname" name="nickname" value="${user.userNickname}"
                       placeholder="昵称">
            </div>
            <span></span>
        </div>
            ...
            <div class="col-sm-5">
                <label class="radio-inline">
                    <input type="radio" name="gender" id="man"
                           value="1" ${user.userGender == "1"? "checked":""} > 男
                </label>
                <label class="radio-inline">
                    <input type="radio" name="gender" id="woman"
                           value="0" ${user.userGender == "0"? "checked":""} > 女
                </label>
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="text" class="form-control" id="phone" name="phone" value="${user.userPhone}"
                       placeholder="手机号码">
            </div>
            <span></span>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <select class="form-control" name="location" id="location" style="width: 150px;">
                    ...
                    <option value="广州" ${user.userAddress == "广州"? "selected":""} >广州</option>
                    <option value="北京" ${user.userAddress == "北京"? "selected":""}>北京</option>
                    <option value="上海" ${user.userAddress == "上海"? "selected":""}>上海</option>
                    <option value="深圳" ${user.userAddress == "深圳"? "selected":""}>深圳</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="radio" name="identity" id="student"
                       value="学生" ${user.userIdentity == "学生"? "checked":""}> 学生&nbsp&nbsp
                <input type="radio" name="identity" id="teacher"
                       value="教师" ${user.userIdentity == "教师"? "checked":""}> 教师&nbsp&nbsp
                <input type="radio" name="identity" id="worker"
                       value="上班族" ${user.userIdentity == "上班族"? "checked":""}> 上班族&nbsp&nbsp
                <input type="radio" name="identity" id="liberal"
                       value="自由职业" ${user.userIdentity == "自由职业"? "checked":""}> 自由职业&nbsp&nbsp
                <input type="radio" name="identity" id="administrators"
                       value="管理员" ${user.userIdentity == "管理员"? "checked":""}> 系统管理员&nbsp&nbsp
            </div>
            <span class="red"></span>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="text" class="form-control" name="detailAddress" id="detailAddress"
                       value="${user.userDetailAddress }" placeholder="详细住址:">
            </div>
            <span class="red"></span>
        </div>
            ...
            <div class="col-sm-3">
                <select class="form-control" name="roleIds" id="roleIds" multiple="multiple" style="width: 150px;">
                    <c:forEach items="${allRoles}" var="role">
                        <option value="${role.characterId}"
                                <c:forEach items="${userRoles}" var="userRole">
                                    ${userRole.characterId == role.characterId ? "selected":""}
                                </c:forEach>
                        >${role.characterName}</option>
                    </c:forEach>
                </select>
