...
<script type="text/javascript">
    ...
    function editUser(userId) {
        location.href = "<%=basePath%>merchants/user/echo/"+userId;
    }

    function deleteUser(userId) {
        if(confirm("真的要删除？")){
            location.href = "<%=basePath%>merchants/user/deletion/"+userId;
        }
    }
    ...
    <table class="table table-hover">
        ...
        <c:forEach items="${users}" var="user" varStatus="vs">
            <tr>
                ...
                <td>${empty user.userGender ? "未设置":(user.userGender == 1?"男":"女")}</td>
                <td>${user.userPhone}</td>
                <td>${user.userEmail}</td>
                <td>${user.userAddress}</td>
                <td>${user.userIdentity}</td>
                <td>${user.whetherOrNoActive == 1?"已激活":"未激活"}</td>
                <td>
