...
<script type="text/javascript">
    ...
    $(function (){
        $('[data-toggle="popover"]').each(function () {
            ...
            $.ajax({
                type:"GET",
                url:"merchants/order/buyer/"+buyerId,
    ...
    function deleteOrder(orderId) {
        if (confirm("确认删除订单吗?")) {
            location.href = "<%=basePath%>merchants/order/deletion/" + orderId;
        }
    }
    ...
    function postOrder(orderId) {
        if (confirm("确认发货吗?")) {
            location.href = "<%=basePath%>merchants/order/post/" + orderId;
        }
    }
    
    function toEditOrder(orderId) {
        location.href = "<%=basePath%>merchants/order/toUpdate/" + orderId;
    }
...
<div class="container" style="border: 1px solid #CCCCCC;height: 1000px;">
    ...
    <table class="table table-hover">
        ...
        <c:forEach items="${orderCustoms}" var="orderCustom" varStatus="vs">
            <tr>
                <td>${vs.count}</td>
                <td>${orderCustom.orderForm.orderFormId}</td>
                <td width="300px">
                ...
                <td>${orderCustom.orderForm.statusString}</td>
                <td class="red">￥${orderCustom.orderForm.amountOfRealPay}</td>
                <td>
                    <button type="button" class="btn btn-xs btn-info" onclick="toEditOrder('${orderCustom.orderForm.orderFormId}')" >
                        查看订单
                    </button>
                    ...
                    <button type="button" id="buyerInfo" class="btn btn-xs btn-warning buyerInfo"
                            ...
                            value="${orderCustom.orderForm.userId}" <%--onclick="findBuyerInfo(${orderCustom.order.userId})"--%>>
                        买家信息
                    </button>
                    <c:if test="${orderCustom.orderForm.orderStatus != 0}">
                        <c:if test="${orderCustom.orderForm.orderStatus < 3}">
                            <button type="button" class="btn btn-xs btn-info" onclick="postOrder('${orderCustom.orderForm.orderFormId}')" >
                                发货
                            </button>
                        </c:if>
