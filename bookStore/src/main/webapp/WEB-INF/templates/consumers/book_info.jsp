...
<%--    </div>--%>
    <div class="row" id="breadcrumb" style="margin-bottom:40px" ;>
        ...
        <span>&gt;</span>
        <a href="book/list?cateId=${bookInfo.bookTypeId}" target="_blank">${bookType.bookTypeName}</a>
        <span>&gt;</span>
        <b>${bookInfo.bookName}</b>
    </div>
    <div class="row">
        <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12" style="height: 400px;">
            <div>
                <a href="book/info?${bookInfo.bookId}">
                    <img src="${bookInfo.bookImageUrl}" width="290px" height="290px"/>
                </a>
            </div>
        </div>
        <div class="col-lg-9 col-md-6 col-sm-12 col-xs-12">
            <div class="name_info">
                <h1 style="font-weight: bold;">${bookInfo.bookName}</h1>
                <h2>
                    <span>${bookInfo.bookOutline}</span>
                </h2>
            </div>
            <div class="book_info_base">
						<span class="book_info_span">
								作者<a href="#">${bookInfo.bookAuthor}</a>著，
								<a href="#">${bookInfo.bookPress}</a>&nbsp;出品
							</span>
                <span class="book_info_span">
								出版社：<a href="" target="_blank">${bookInfo.bookPress}</a>
							</span>
                <span class="book_info_span">
								出版日期：<fmt:formatDate value="${bookInfo.bookPublishDate}" pattern="yyyy-MM-dd"/>
							</span>
            </div>
            <div class="book_price">
                <p class="what_price">特价</p>
                <p class="price_info">
                    <span class="rob" style="font-size: 30px;">￥${bookInfo.bookPrice}</span>
                    <span style="color: red;">(${bookInfo.bookDiscount}折扣)</span>
                    <span>定价:</span>
                    <span class="oprice">￥${bookInfo.bookMarketPrice}</span>
                </p>
            </div>
                    ...
                    <c:forEach items="${recommendBookList}" var="recommendBook">
                        <c:if test="${recommendBook.bookId != bookInfo.bookId}">
                            <li class="product_li">
                                ...
                                <p class="price">
                                    <span class="rob">￥${recommendBook.bookPrice}</span>
                                    <span class="oprice">￥${recommendBook.bookMemberPrice}</span>
                                </p>
                                <p class="name">
                                    <a href="book/info?${recommendBook.bookId}">${recommendBook.bookName}</a>
                                </p>
                                <p class="author">${recommendBook.author} 著，${recommendBook.bookPress} 出</p>
                                </p>
                            </li>
                        </c:if>
                    </c:forEach>
        ...
        <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
            ...
            <div class="content">
                <div id="book_detail_content" class="nav_content">
                    <ul id="key">
                        <li>开 本：${bookInfo.bookSize}</li>
                        <li>纸 张：胶版纸</li>
                        <li>包 装：${bookInfo.bookPackStyle}</li>
                        <li>是否套装：否</li>
                        <li>国际标准书号ISBN：${bookInfo.bookIsbn}</li>
                    </ul>
