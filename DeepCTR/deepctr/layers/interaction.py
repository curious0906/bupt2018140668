...
from tensorflow.python.keras.initializers import (Zeros, glorot_normal,
                                                  glorot_uniform, RandomNormal)
from tensorflow.python.keras.layers import Layer
from tensorflow.python.keras.regularizers import l2
from tensorflow.python.layers import utils

from .activation import activation_layer
from .utils import concat_fun, reduce_sum, softmax, reduce_mean, normalize
...
class AutoIntInteractingLayer(Layer):
    """A Layer used in AutoInt that model the correlations between different feature fields by multi-head self-attention mechanism.
    ...
    def __init__(self, att_embedding_size=8, head_num=2, use_res=True, seed=1024, dropout_prob=0, is_training=True, **kwargs):
        if head_num <= 0:
            raise ValueError('head_num must be a int > 0')
        self.att_embedding_size = att_embedding_size
        self.head_num = head_num
        ...
        super(AutoIntInteractingLayer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        units = self.att_embedding_size * self.head_num
        self.query_layer = tf.keras.layers.Dense(units, activation=tf.nn.relu)
        self.key_layer = tf.keras.layers.Dense(units, activation=tf.nn.relu)
        self.value_layer = tf.keras.layers.Dense(units, activation=tf.nn.relu)
        if self.use_res:
            self.res_layer = tf.keras.layers.Dense(units, activation=tf.nn.relu)

        # normalize parameters
        self.beta = tf.Variable(tf.zeros(units), name="beta")
        self.gamma = tf.Variable(tf.ones(units), name="gamma")

        # Be sure to call this somewhere!
        super(AutoIntInteractingLayer, self).build(input_shape)
    ...
    def call(self, inputs, **kwargs):
        ...
        # Split and concat
        Q_ = tf.concat(tf.split(queries, self.head_num, axis=2), axis=0)
        K_ = tf.concat(tf.split(keys, self.head_num, axis=2), axis=0)
        V_ = tf.concat(tf.split(values, self.head_num, axis=2), axis=0)
        ...
        # Dropouts
        # weights = tf.layers.dropout(weights, rate=self.dropout_prob,
        ...
        # Restore shape
        outputs = tf.concat(tf.split(outputs, self.head_num, axis=0), axis=2)

        # Residual connection
        if self.use_res:
            V_res = self.res_layer(inputs)
            outputs += V_res
    ...
    def _normalize(self, inputs, epsilon=1e-8):
        '''
    ...
    def get_config(self, ):
        config = {'att_embedding_size': self.att_embedding_size, 'head_num': self.head_num, 'use_res': self.use_res,
                  'seed': self.seed, "dropout_prob": self.dropout_prob, "is_training": self.is_training}
        base_config = super(AutoIntInteractingLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
